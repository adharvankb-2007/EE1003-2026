...
\[
\frac{F_0 \omega}{(s^2 + \omega^2)(m s^2 + c s + k)}
=
\frac{A s + B}{s^2 + \omega^2}
+
...
%--- Slide 6: Inverse Laplace Transform ---
\begin{frame}{Inverse Laplace Transform}
    Applying $\mathcal{L}^{-1}$ to the steady-state term $\dfrac{A}{s - i\omega}$:
    \[
        \mathcal{L}^{-1}\left\{\frac{A}{s - i\omega}\right\} = A\, e^{i\omega t}
    \]
    Substituting $A = \dfrac{F_0}{(k - m\omega^2) + ic\omega}$:
    \[
        x_{ss}(t) = \frac{F_0}{(k - m\omega^2) + ic\omega} \cdot e^{i\omega t}
    \]
    Writing the denominator in polar form $r e^{i\phi}$ where:
    \[
        r = \sqrt{(k-m\omega^2)^2 + (c\omega)^2}, \quad
        \phi = \arctan\!\left(\frac{c\omega}{k - m\omega^2}\right)
    \]
\end{frame}

%--- Slide 7: Final Steady-State Expression ---
\begin{frame}{Final Steady-State Response}
    Therefore:
    \[
        x_{ss}(t) = \frac{F_0}{\sqrt{(k - m\omega^2)^2 + (c\omega)^2}} \cdot e^{i(\omega t - \phi)}
    \]
    Taking the \textbf{real part} for a cosine excitation:
    \[
        \boxed{x_{ss}(t) = X \cos(\omega t - \phi)}
    \]
    where:
    \[
